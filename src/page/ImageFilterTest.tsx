import React, {useCallback, useState} from 'react';
import {SafeAreaView, TouchableOpacity, View, Text} from 'react-native';
import {MainParamList} from '~/router/router.types';
import {NativeStackScreenProps} from '@react-navigation/native-stack';
import {FilterImage, Loading} from '~/components';
import {getFont, getWidth} from '~/libs/crossDevice';
import {ScrollView} from 'react-native-gesture-handler';
import {filterArray} from '~/Utils/ImageFilter';
type ImageFilterTest = NativeStackScreenProps<MainParamList, 'ImageFilterTest'>;
interface Props {
  navigation: ImageFilterTest['navigation'];
  route: ImageFilterTest['route'];
}

const ImageFilterTest = ({route: {params}}: Props) => {
  const [imageFilterType, setImageFilterType] = useState({
    title: '기본',
    FilterComponent: 'Normal',
  });
  const [imageUrl] = useState(params?.imageUrl ?? null);

  //저장된 이미지 저장
  const [saveFilterImage, setSaveFilterImage] = useState<
    string | undefined | null
  >(params?.imageUrl ?? null);
  const [loading, setLoading] = useState(false);

  //선택한 이미지 필터 변경
  const EditFilterSaveImage = useCallback(
    (filterImage: string | undefined | null) => {
      setSaveFilterImage(filterImage);
      setLoading(false);
    },
    [],
  );
  console.log('saveFilterImage :: ', saveFilterImage);
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: '#fff'}}>
      <View style={{flex: 1}}>
        {imageUrl && (
          <FilterImage
            url={imageUrl}
            imageFilterType={imageFilterType?.FilterComponent}
            EditFilterSaveImage={EditFilterSaveImage}
            extractImageEnabled={true}
          />
        )}
      </View>

      {params?.type === 'free' && (
        <View style={{width: '100%', marginTop: 10, marginBottom: 20}}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View
              style={{paddingHorizontal: getWidth(10), flexDirection: 'row'}}>
              {filterArray.map((element, index) => {
                return (
                  <TouchableOpacity
                    key={index}
                    onPress={() => {
                      if (
                        imageFilterType.FilterComponent ===
                        element.FilterComponent
                      ) {
                        return;
                      } else {
                        setLoading(true);
                        setImageFilterType(element);
                      }
                    }}
                    style={{
                      marginRight: getWidth(10),
                      alignItems: 'center',
                    }}>
                    <Text
                      style={{
                        fontSize: getFont(12),
                        color: '#222',
                        marginBottom: 5,
                      }}>
                      {element.FilterComponent}
                    </Text>
                    <View
                      style={{
                        width: getWidth(120),
                        height: getWidth(120),
                      }}>
                      {/* url이미지가 아닌 단일 이미지 적용 */}
                      <FilterImage
                        imageFilterType={element.FilterComponent}
                        url={
                          'data:image/jpeg;base64,'
                        }
                      />
                    </View>
                  </TouchableOpacity>
                );
              })}
            </View>
          </ScrollView>
        </View>
      )}
      {loading && <Loading />}
    </SafeAreaView>
  );
};

export default ImageFilterTest;
