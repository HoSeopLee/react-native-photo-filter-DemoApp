import {
  Pressable,
  SafeAreaView,
  Text,
  TouchableOpacity,
  View,
} from 'react-native';
import React, {useCallback, useState} from 'react';
import {FilterImage, Loading} from '~/components';
import {getFont, getWidth} from '~/libs/crossDevice';
import {ScrollView} from 'react-native-gesture-handler';
import {filterArray} from '~/Utils/ImageFilter';
import {getFontFamily} from '~/libs/globalFontStyles';

interface Props {
  modal_state: (value: boolean) => void;
  EditChoice?: (value: string | undefined | null, key: number) => void;
  image_Data: any;
  keyValue: number;
}

const ImageSelectFilterModal = ({
  modal_state = () => {},
  EditChoice = () => {},
  image_Data = null,
  keyValue = 0,
}: Props) => {
  const [imageValue] = useState(image_Data);
  const [imageFilterType, setImageFilterType] = useState({
    title: '기본',
    FilterComponent: 'Normal',
  });
  const [saveFilterImage, setSaveFilterImage] = useState<
    string | undefined | null
  >('');
  const [loading, setLoading] = useState(false);
  //선택한 이미지 필터 변경
  const EditFilterSaveImage = useCallback(
    (filterImage: string | undefined | null) => {
      setSaveFilterImage(filterImage);
      setLoading(false);
    },
    [],
  );
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: '#fff'}}>
      <View
        style={{
          width: '100%',
          height: 50,
          paddingHorizontal: getWidth(16),
          alignItems: 'center',
          flexDirection: 'row',
        }}>
        <Pressable
          style={({pressed}) => [
            {
              width: 'auto',
              opacity: pressed ? 0.5 : 1,
            },
          ]}
          onPress={async () => {
            modal_state(false);
          }}>
          <Text
            style={{
              fontSize: getFont(16),
              fontFamily: getFontFamily.Pretendard_Regular,
              fontWeight: '500',
              color: '#181818',
            }}>
            취소
          </Text>
        </Pressable>
        <View style={{flex: 1, alignItems: 'flex-end'}}>
          <Pressable
            style={({pressed}) => [
              {
                flexDirection: 'row',
                alignItems: 'center',

                opacity: pressed ? 0.5 : 1,
              },
            ]}
            onPress={async () => {
              modal_state(false);
              EditChoice(saveFilterImage, keyValue);
            }}>
            <Text
              style={{
                fontSize: getFont(16),
                fontFamily: getFontFamily.Pretendard_Regular,
                fontWeight: '500',
                color: '#181818',
              }}>
              완료
            </Text>
          </Pressable>
        </View>
      </View>
      <View style={{flex: 1}}>
        {image_Data && (
          <FilterImage
            url={imageValue}
            imageFilterType={imageFilterType?.FilterComponent}
            EditFilterSaveImage={EditFilterSaveImage}
            extractImageEnabled={true}
          />
        )}
      </View>
      <View style={{width: '100%', marginTop: 10, marginBottom: 20}}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={{paddingHorizontal: getWidth(10), flexDirection: 'row'}}>
            {filterArray.map((element, index) => {
              return (
                <TouchableOpacity
                  key={index}
                  onPress={() => {
                    if (
                      imageFilterType.FilterComponent ===
                      element.FilterComponent
                    ) {
                      return;
                    } else {
                      setLoading(true);
                      setImageFilterType(element);
                    }
                  }}
                  style={{
                    marginRight: getWidth(10),
                    alignItems: 'center',
                  }}>
                  <Text
                    style={{
                      fontSize: getFont(12),
                      color: '#222',
                      marginBottom: 5,
                    }}>
                    {element.FilterComponent}
                  </Text>
                  <View
                    style={{
                      width: getWidth(120),
                      height: getWidth(120),
                    }}>
                    {/* url이미지가 아닌 단일 이미지 적용 */}
                    <FilterImage
                      imageFilterType={element.FilterComponent}
                      url={
                        'data:image/jpeg;base64,'
                      }
                    />
                  </View>
                </TouchableOpacity>
              );
            })}
          </View>
        </ScrollView>
      </View>
      {loading && <Loading />}
    </SafeAreaView>
  );
};

export default ImageSelectFilterModal;
